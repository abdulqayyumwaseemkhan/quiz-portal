      ...
      <main className="flex-1 ml-64 p-10">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-50 flex justify-between items-center">
            <h2 className="text-2xl font-black text-gray-800 italic">Course Catalog</h2>
            <div className="flex gap-2">
                <span className="bg-primary-50 text-primary-600 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest">{quizzes.length} ACTIVE</span>
            </div>
          </div>
                ...
                {quizzes.map((quiz) => (
                  <tr key={quiz._id} className="hover:bg-gray-50/50 transition-all group">
                    <td className="px-8 py-8">
                       <p className="font-black text-lg text-gray-800 group-hover:text-primary-600 transition-colors uppercase tracking-tighter">{quiz.title}</p>
                    ...
                    <td className="px-8 py-8">
                       <div className="bg-gray-100 h-12 w-12 rounded-2xl flex items-center justify-center">
                          <span className="text-lg font-black text-gray-800">{quiz.totalMarks}</span>
                       </div>
                    </td>
                    <td className="px-8 py-8 text-right pr-12">
                        <div className="flex justify-end gap-3 transition-all duration-300">
                            <button 
                              ...
                              className={`p-3 rounded-2xl shadow-sm border border-gray-100 transition-all ${
                            ...
                            <Link 
                              ...
                              className="p-3 bg-white text-primary-600 hover:bg-primary-50 rounded-2xl shadow-sm border border-gray-100 transition-all"
                            ...
                            <Link 
                              ...
                              className="p-3 bg-white text-amber-500 hover:bg-amber-50 rounded-2xl shadow-sm border border-gray-100 transition-all"
                              title="Edit Details"
                            >
                              <Edit size={20} />
                            </Link>
                            <button 
                              ...
                              className="p-3 bg-white text-red-500 hover:bg-red-50 rounded-2xl shadow-sm border border-gray-100 transition-all"
                              title="Delete Quiz"
                            >
                              <Trash2 size={20} />
                            </button>
          ...
          {quizzes.length === 0 && !loading && (
            <div className="p-24 text-center">
               ...
               <p className="text-gray-400 font-black text-xl uppercase tracking-widest">Repository is Empty</p>
...
const StatCard = ({ title, value, icon: Icon, color, bg }) => (
  <motion.div 
    whileHover={{ y: -8 }}
    className="card flex items-center gap-6 p-10 bg-white border-none shadow-xl shadow-gray-200/50 rounded-[2.5rem]"
  >
    <div className={`p-5 rounded-[1.5rem] ${bg} ${color} shadow-inner`}>
      <Icon size={32} />
    </div>
    <div>
      <p className="text-gray-400 text-xs font-black uppercase tracking-[0.2em] mb-1">{title}</p>
