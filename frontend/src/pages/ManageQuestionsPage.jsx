      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <header className="flex justify-between items-end mb-10">
          <div>
            ...
            <p className="text-gray-500">{questions.length} Questions Added So Far</p>
          </div>
          <button 
          ...
          <form onSubmit={handleAddQuestion} className="card bg-white p-8 mb-10 border-2 border-primary-100 animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            {newQuestion.type === 'mcq' ? (
              <div className="space-y-4 mb-6">
                <label className="block text-sm font-bold text-gray-700">Options (Select the correct one)</label>
              ...
              <div className="mb-6">
                <label className="block text-sm font-bold text-gray-700 mb-2">Correct Answer (Exact Match)</label>
                <input
                  type="text"
                  className="input-field"
        ...
        <div className="space-y-6">
            ...
            <div key={q._id} className="card relative group hover:border-primary-300 transition-all border-l-4 border-l-primary-600">
               <button 
                ...
                <div className="flex items-start gap-4">
                  <div className="bg-primary-50 text-primary-700 w-10 h-10 rounded-full flex items-center justify-center font-bold flex-shrink-0">
                    {idx + 1}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                       <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-[10px] font-bold uppercase">{q.type}</span>
                       <span className="text-gray-400 text-xs font-medium">{q.marks} Marks</span>
                    </div>
                    <h4 className="text-lg font-semibold text-gray-800 pr-10">{q.questionText}</h4>
                    
                    {q.type === 'mcq' && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                        {q.options.map((opt, i) => (
                          <div key={i} className={`flex items-center gap-2 text-sm p-3 rounded-lg border ${q.correctAnswer === opt ? 'bg-green-50 border-green-200 text-green-700 font-bold' : 'bg-gray-50 border-gray-100 text-gray-600'}`}>
                    ...
                    {q.type === 'short' && (
                       <div className="mt-4 p-3 bg-green-50 border border-green-100 rounded-lg text-sm text-green-700 font-medium">
          ...
          {questions.length === 0 && !showAdd && (
            <div className="text-center py-20 bg-white rounded-2xl border-2 border-dashed border-gray-200">
               ...
               <p className="text-gray-500 font-medium text-lg">Empty Quiz</p>
               <p className="text-gray-400">Click the button above to add your first question.</p>
            </div>
          )}
        </div>
      </main>
